...
export function datetimeLocal(iso:string,minuteStep=1){const d=new Date(iso);const minute=Math.floor(d.getMinutes()/minuteStep)*minuteStep;return `${dayKey(d)}T${String(d.getHours()).padStart(2,'0')}:${String(minute).padStart(2,'0')}`;}
