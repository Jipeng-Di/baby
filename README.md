...
注册并登录 → 创建家庭 → 在“家庭”添加宝宝 → 在“今天”点 `+60/+90/+120` → 检查类型和时间 → 保存。设备会按宝宝记住上次选择的奶类型。同一个已验证账号可在多台设备同时登录并共享数据；退出登录只结束当前设备会话。家庭页生成 7 天有效的邀请链接；其他用户注册/登录后打开链接即可加入。加入者可查看、添加、编辑和删除该家庭记录。
